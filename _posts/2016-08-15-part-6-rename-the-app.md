...
If you want to transform our Thun-Demo application into a digital editions dealing with sources about reading in alpine regions in the 18th/19th century and which might be called Reading in the alps, then go ahead and change **expath-pkg.xml** into:
...
The only thing to do now, is to change **repo.xml** an eXist-db specific supplementation of the (standardized) expath-pkg.xml. 
...
Now we can download the application by clicking on Application/Download button in eXide’s navigation menu (make sure you opened any file from the application). 

# Deploy and test

Now we can install this packages and see if everything works at it should or if we still find traces of "thun-demo" in our brand new “rita-demo”. 
